...
Feature Selection 
-----------------
From the `README` and `features.txt` files in the original dataset mentioned above, the features are as follow:
...
* `mean`: Mean value (`mean()` in the oringinal dataset)
* `std`: Standard deviation  (`std()` in the oringinal dataset)
